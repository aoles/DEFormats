...
We provide data conversion functions of the form `as.<class>(object)`, where `<class>` is the name of the class to which `object` should be coerced. Aditionally, we define constructors for common count data input formats, e.g. *SummarizedExperiment* objects.
...
and construct a sample RNA seq experiment dataset.
...
We will now use this count matrix to contruct a `DGEList` objects.

```{r dge}
dge = DGEList(counts, group = colnames(counts))
dge
```
...
dds1 = DESeqDataSetFromMatrix(counts, data.frame(condition = colnames(counts)), ~ condition)
dds2 = DESeqDataSetFromMatrix(counts, data.frame(condition = colnames(counts)), ~ condition)
...
Let's create a sample expression dataset of an RNA-seq experiment.
...
Similarily as for the *DESeqDataSet* objects in `r Biocpkg("DESeq2")`,
...
```{r dgeSE}
dge = DEFormats::DGEList(se, group = colnames(se))
dge
```
